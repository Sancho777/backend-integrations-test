...
PROJECT_DIR = '/home/Adria/Documentos/Projetos/CornerShop'
...
def process_csv_files():
    ...
    prices_stock_df = prices_stock_df[(prices_stock_df.BRANCH == 'MM') | (
        prices_stock_df.BRANCH == 'RHSM')]
    products_df['DESCRIPTION'] = products_df['DESCRIPTION'].str.replace(
        '<[^<]+?>', '')
    products_df['CATEGORY'] = products_df['CATEGORY'].str.lower(
    ) + '|' + products_df['SUB_CATEGORY'].str.lower()
    ...
    products_df['package'], products_df['DESCRIPTION'] = products_df.apply(
        package_extraction, axis=1)
   ...
   # Start a Session
    ...
    session.bulk_insert_mappings(
        Product, products_df.to_dict(orient="records"))
    session.commit()
    ...
    joined_table = pd.merge(prices_stock_df, products_data,
    ...
    session.bulk_insert_mappings(
        BranchProduct, joined_table.to_dict(orient="records"))
