...
class CaWalmartSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        next_page_button_link = response.xpath(
            '//a[@analytics-data="next results page"]/@href').get()
        if next_page_button_link:
            next_page_button_link = response.urljoin(next_page_button_link)
            yield scrapy.Request(url=next_page_button_link, headers=self.headers, cookies=self.cookies, callback=self.parse_page, cb_kwargs={'products': products})

    def parse_page(self, response, **kwargs):
        # get next page urls
        if kwargs:
            products = kwargs['products']
        products.extend(response.xpath(
            '//a[@class="product-link"]/@href').getall())
        for product in products:
            product_url = self.root_url + product
            yield scrapy.Request(url=product_url, headers=self.headers, cookies=self.cookies, callback=self.parse_product)

    def parse_product(self, response):
        # Extracts Productpages Data
        ProductItems = ProductItem()

        isFruit = response.xpath(
            '//li[@data-automation="desktop-breadcrumb-item-3"]/a[@class="css-wkrwfv elkyjhv0"]/text()').get().upper()
        if isFruit == "FRUITS":
            category = response.xpath(
                '//a[@class="css-wkrwfv elkyjhv0"]/text()').get()
            ProductItems['category'] = category.replace(",", "|")
            ProductItems['brand'] = response.xpath(
                '//script').re(re.compile(r'\"brand\":\{\"name\"\:\"(.*?)\"\}', re.MULTILINE | re.DOTALL))[0]
            ProductItems['sku'] = response.xpath(
                '//script').re(re.compile(r'\"activeSkuId\":\"(.*?)\"', re.MULTILINE | re.DOTALL))[0]
            barcodes = response.xpath(
                '//script').re(re.compile(r'\"upc\":(\[.*?\])', re.MULTILINE | re.DOTALL))[0]
            ProductItems['barcodes'] = barcodes[0]
            ProductItems['name'] = response.xpath(
                '//h1[@class="css-1c6krh5 e1yn5b3f6"]/text()').get()
            ProductItems['description'] = response.xpath(
                '//div[@data-automation="long-description"]/text()').get()

            ProductItems['package'] = response.xpath(
                '//p[@data-automation="short-description"]/text()').get()
            ProductItems['store'] = 'Walmart'
            ProductItems['url'] = response.url
            ProductItems['image_url'] = response.xpath(
                '//div[@role="presentation"]/img/@src').get()

            ProductItems['price'] = response.xpath(
                '//span[@data-automation="buybox-price"]/text()').get()
            ProductItems['stock'] = 'TBD'
            #ProductItems['branch'] = 'ThunderBay Supercenter'

            postal_code = response.xpath(
                '//*[@id="find-in-store-section"]//form//input/@value').get()

            if postal_code == 'P7B3Z7':  # Store 3124
                ProductItems['branch'] = '3124'
            elif postal_code == 'M6H4A9':  # M6H4A9 Store 3106
                ProductItems['branch'] = '3106'
            else:
                ProductItems['branch'] = 'Other'
